...
public class PolygonPath extends Path
{
    ...
    public void write_scope(IndentFileWriter p_file, IdentifierType p_identifier_type) throws java.io.IOException
    {
        ...
        p_file.write(" ");
        p_file.write((Double.valueOf(this.width)).toString());  //Ontobus
        int corner_count = coordinate_arr.length/ 2;
        for (int i = 0; i < corner_count; ++i)
        {
            p_file.new_line();
            p_file.write(Double.valueOf(coordinate_arr[2 * i]).toString());  //Ontobus
            p_file.write(" ");
            p_file.write(Double.valueOf(coordinate_arr[2 * i + 1]).toString());  //Ontobus
        }
        p_file.end_scope();
    }
